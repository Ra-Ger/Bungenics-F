...
public class VetView extends VerticalLayout {
    ...
    public VetView(BackendClientService backendClientService, PlayerSession playerSession) {
    ...
    private void configureGeneralGrid() {
        ...
        generalRabbitsGrid.addComponentColumn(rabbit -> {
            ...
            boolean isEligibleStatus = "IDLE".equalsIgnoreCase(rabbit.status()) || "RESTING".equalsIgnoreCase(rabbit.status());
            admitBtn.setEnabled(isEligibleStatus && (currentHp < maxHp || currentStress > 0.0f));
