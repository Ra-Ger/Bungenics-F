...
public class ShopView extends VerticalLayout {
    ...
    public ShopView(BackendClientService backendClientService, PlayerSession playerSession) {
        ...
        Button buyFoodBtn = new Button("Buy Food", e -> {
            ...
            if (playerId != null && foodTypeCombo.getValue() != null) {
                try {
                    backendClientService.buyFood(playerId, foodTypeCombo.getValue(), amountField.getValue().floatValue());
        ...
        Button sellBtn = new Button("Sell Selected Rabbit", e -> {
            Long playerId = playerSession.getPlayerId();
            if (playerId != null && rabbitSellCombo.getValue() != null) {
                try {
                    backendClientService.sellRabbit(playerId, rabbitSellCombo.getValue().id());
    ...
    private void configureMarketGrid() {
        ...
        marketGrid.addComponentColumn(rabbit -> {
            ...
            buyBtn.addClickListener(e -> {
                ...
                if (playerId != null) {
                    try {
                        backendClientService.buyRabbit(playerId, rabbit.id());
    ...
    private void refreshPlayerData() {
        try {
            Long playerId = playerSession.getPlayerId();
            if (playerId != null) {
                PlayerDto p = backendClientService.getPlayer(playerId);
