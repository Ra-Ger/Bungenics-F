...
public class StartMenuView extends VerticalLayout {
    ...
    public StartMenuView(BackendClientService backendService) {
        ...
        if (VaadinSession.getCurrent().getAttribute("playerId") != null) {
    ...
    private void openNewGameDialog() {
        ...
        com.vaadin.flow.component.combobox.ComboBox<String> locationCombo = new com.vaadin.flow.component.combobox.ComboBox<>("Select Farm Location");
        ...
        com.vaadin.flow.component.html.Paragraph descPara = new com.vaadin.flow.component.html.Paragraph();
        ...
        Button createBtn = new Button("Create & Start", e -> {
            ...
            if (!name.isEmpty() && location != null && !location.isEmpty()) {
                try {
                    PlayerDto newPlayer = backendService.createPlayer(name, location);
                    backendService.createFarm(newPlayer.id());

                    VaadinSession.getCurrent().setAttribute("playerId", newPlayer.id());
                    UI.getCurrent().navigate(FarmView.class);
                    dialog.close();
                } catch (Exception ex) {
                    Notification.show("Failed to create player or farm: " + ex.getMessage());
                }
            } else {
                Notification.show("Name and Location cannot be empty.");
            }
        });
    ...
    private void openLoadGameDialog() {
        ...
        Button loadBtn = new Button("Load & Play", VaadinIcon.PLAY.create(), e -> {
            if (playerCombo.getValue() != null) {
                VaadinSession.getCurrent().setAttribute("playerId", playerCombo.getValue().id());
                UI.getCurrent().navigate(FarmView.class);
                dialog.close();
            } else {
        ...
        // Red delete button to cascade wipe players and all associated data
        Button deleteBtn = new Button("Delete Profile", VaadinIcon.TRASH.create(), e -> {
            ...
            if (selected != null) {
                try {
                    ...
                    populateDropdown.run(); // refresh dropdown items
                    playerCombo.setValue(null);
                } catch (Exception ex) {
                    Notification.show("Wipe failed: " + ex.getMessage());
                }
            } else {
